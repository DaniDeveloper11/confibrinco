...
<template>
  ...
  <!-- ═══════════════════════════════════════ CATÁLOGO ══ -->
  <section id="catalogo" class="py-16 bg-brand-soft sm:py-24">
    <div class="mx-auto max-w-7xl px-5 sm:px-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        >
          <!-- Color banner -->
          <div
            class="h-64 grid place-items-center overflow-hidden sm:h-44"
  ...
  <!-- ════════════════════════════════════════════ PROCESO ══ -->
  <section id="proceso" class="py-16 bg-white sm:py-24">
    <div class="mx-auto max-w-7xl px-5 sm:px-6">
  ...
  <!-- ═══════════════════════════════════════════ SECTORES ══ -->
  <section id="sectores" class="py-16 bg-dark sm:py-24">
    <div class="mx-auto max-w-7xl px-5 sm:px-6">

      <div class="mx-auto mb-10 max-w-2xl text-center sm:mb-14">
        <span class="mb-3 inline-block text-sm font-semibold uppercase tracking-widest text-brand">Soluciones a medida</span>
        <h2 class="text-2xl font-bold tracking-tight text-white sm:text-3xl sm:text-4xl">
          ¿Para quién <span class="text-brand">endulzamos?</span>
        </h2>
        <p class="mt-4 text-sm text-white/50 sm:text-base">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 sm:gap-5">
        <div
          v-for="s in sectores"
          :key="s.titulo"
          class="group relative overflow-hidden rounded-2xl bg-surface p-6 ring-1 ring-white/5 transition hover:ring-brand/40 sm:rounded-3xl sm:p-8"
        >
          <!-- Badge "Mayor demanda" -->
          <span
            v-if="s.badge"
          ...
          <div class="flex items-start gap-4 sm:block">
            ...
            <div>
              <h3 class="font-bold text-white mb-2 sm:mb-3">{{ s.titulo }}</h3>
  ...
  <!-- ══════════════════════════════════════════════ STATS ══ -->
  <section class="py-16 sm:py-20" style="background: linear-gradient(135deg, #F29F05 0%, #F28705 50%, #D93D04 100%)">
    <div class="mx-auto max-w-7xl px-5 sm:px-6">
  ...
  <!-- ══════════════════════════════════════ TESTIMONIOS ══ -->
  <section id="testimonios" class="py-16 bg-brand-soft sm:py-24">
    <div class="mx-auto max-w-7xl px-5 sm:px-6">
  ...
  <!-- ═══════════════════════════════════════════ CONTACTO ══ -->
  <section id="contacto" class="py-16 bg-dark sm:py-24">
    <div class="mx-auto max-w-7xl px-5 sm:px-6">
      <div class="grid gap-12 lg:grid-cols-2 lg:gap-16 items-start">
        ...
        <div>
          ...
          <p class="mt-4 text-sm text-white/60 leading-relaxed max-w-md sm:mt-5 sm:text-base">
            Nuestro equipo de diseño te asesora sin costo. Cuéntanos tu necesidad
            y en menos de <strong class="text-white">24 horas</strong> tendrás
          ...
          <ul class="mt-8 space-y-4 text-sm sm:mt-10 sm:space-y-5">
            <li class="flex items-center gap-3 text-white/70">
              <span class="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-brand/15 text-brand">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z"/>
                </svg>
              </span>
              <div>
                <p class="font-medium text-white">Ventas & WhatsApp</p>
                <p>+52 33 1234 5678</p>
              </div>
            </li>
            <li class="flex items-center gap-3 text-white/70">
            ...
            <li class="flex items-center gap-3 text-white/70">
              ...
              <div>
                <p class="font-medium text-white">Showroom & Fábrica</p>
        ...
        <!-- Formulario -->
        <div class="rounded-2xl bg-surface p-6 ring-1 ring-white/10 sm:rounded-3xl sm:p-8">

          <!-- Éxito -->
          <div v-if="enviado" class="flex flex-col items-center justify-center gap-4 py-12 text-center">
            <div class="text-5xl">🍬</div>
            <h3 class="text-xl font-bold text-white">¡Recibido!</h3>
          ...
          <form v-else @submit="enviar" novalidate class="flex flex-col gap-4 sm:gap-5">
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-5">
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/40">Nombre completo</label>
                <input
                  v-model="form.nombre"
                  type="text"
                  required
                  placeholder="Ej. Juan Pérez"
                  class="w-full rounded-xl bg-white/5 px-4 py-3.5 text-sm text-white placeholder-white/20 ring-1 ring-white/10 focus:outline-none focus:ring-brand"
                />
              </div>
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/40">Empresa</label>
                <input
            ...
            <div>
              <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/40">Correo corporativo</label>
              <input
                v-model="form.email"
                type="email"
                required
            ...
            <div>
              <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/40">¿Qué producto te interesa?</label>
              <select
                v-model="form.producto"
                class="w-full rounded-xl bg-white/5 px-4 py-3.5 text-sm text-white ring-1 ring-white/10 focus:outline-none focus:ring-brand appearance-none"
              >
                <option value="" class="bg-dark" disabled>Selecciona un producto</option>
                <option value="caramelos"  class="bg-dark">Caramelos</option>
                <option value="gomitas"    class="bg-dark">Gomitas</option>
                <option value="paletas"    class="bg-dark">Paletas</option>
                <option value="obleas"     class="bg-dark">Obleas</option>
                <option value="combo"      class="bg-dark">Combinación / no sé aún</option>
              </select>
            </div>

            <div>
              <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/40">Cuéntanos tu proyecto</label>
              <textarea
                v-model="form.mensaje"
                rows="4"
                placeholder="Cantidad aproximada, tipo de evento o negocio, fecha de entrega…"
                class="w-full rounded-xl bg-white/5 px-4 py-3.5 text-sm text-white placeholder-white/20 ring-1 ring-white/10 focus:outline-none focus:ring-brand resize-none"
              ></textarea>
            </div>
